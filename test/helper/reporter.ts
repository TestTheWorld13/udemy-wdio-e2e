...
function addStep(testid: string, loglevel: string, msg: string, toAllure = true, issueid = undefined) {
    ...
    try {
        if (loglevel === "info") logger.info(`[${testid}]: ${msg}`) // used to get information about the steps
        if (loglevel === "debug") logger.debug(`[${testid}]: ${msg}`) // use when, some informaiton is critical for debug
        if (loglevel === "warn") logger.warn(`[${testid}]: ${msg}`) // use when, if something may result in a crash
        if (loglevel === "error") { // use when, report the error then move on instead of failing steps 
